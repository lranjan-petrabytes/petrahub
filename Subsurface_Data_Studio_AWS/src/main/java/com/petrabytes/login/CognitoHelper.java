...
class CognitoHelper {
	...
	InitiateAuthResult siginUSer(String username, String password) {
		AuthenticationHelper helper = new AuthenticationHelper(POOL_ID, CLIENTAPP_ID, CLIENT_SECRET);
		return helper.PerformSMSAuthentication(username, password);
	}

	AuthenticationResultType mfavalid(String username, String mfaCode, String session) {
		AuthenticationHelper helper = new AuthenticationHelper(POOL_ID, CLIENTAPP_ID, CLIENT_SECRET);
		return helper.mfaLogin(username, mfaCode, session);
	}
	 ...
	 * @return returns the credentials based on the access token returned from the
	 *         user pool.
	 */
	Credentials GetCredentials(String idprovider, String id) {
		AnonymousAWSCredentials awsCreds = new AnonymousAWSCredentials();
		AmazonCognitoIdentity provider = AmazonCognitoIdentityClientBuilder.standard()
				.withCredentials(new AWSStaticCredentialsProvider(awsCreds)).withRegion(Regions.fromName(REGION))
				.build();
	 ...
	 * @param code     code sent for password reset from the ResetPassword() method
	 *                 above
	 * @return returns code delivery details
	 */
