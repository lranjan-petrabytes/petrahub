...
class AuthenticationHelper {
	...
	InitiateAuthResult PerformSMSAuthentication(String username, String password) {
		String authresult = null;
...
//        InitiateAuthRequest initiateAuthRequest = initiateUserSrpAuthRequest(username);
		try {
			...
			InitiateAuthRequest initiateAuthRequest = new InitiateAuthRequest().withClientId(clientId)
					.withAuthFlow(AuthFlowType.USER_PASSWORD_AUTH).withAuthParameters(authParams);

			final InitiateAuthResult result = cognitoIdentityProvider.initiateAuth(initiateAuthRequest);
			System.out.println("Result Challange is :  " + result.getChallengeName());
			return result;
		} catch (final Exception ex) {
			System.out.println("Exception" + ex);
		//	ex.printStackTrace();
	  ...
	  AuthenticationResultType mfaLogin(String username, String mfaCode, String session) {
		...
		RespondToAuthChallengeRequest respondToAuthChallengeRequest = new RespondToAuthChallengeRequest()
				.withChallengeName(ChallengeNameType.SMS_MFA).withClientId(this.clientId)
				.withChallengeResponses(challengeResponses).withSession(session);

		RespondToAuthChallengeResult respondToAuthChallengeResult = cognitoIdentityProvider
				.respondToAuthChallenge(respondToAuthChallengeRequest);
		System.out.println("User logged in Cognito user pool using SMS MFA."+respondToAuthChallengeResult.getAuthenticationResult());
